...
send_alert() {
  ...
  if [[ -n "$ALERT_HOOK" ]]; then
    # 正文作为参数传入，避免错误信息里的引号、换行被当成 Python 源码。
    local payload
    payload=$(python3 -c '
import json, sys
print(json.dumps({
    ...
    "card": {
        "header": {"title": {"tag": "plain_text", "content": sys.argv[1]}, "template": "red"},
        "elements": [{"tag": "div", "text": {"tag": "lark_md", "content": sys.argv[2]}}]
    }
}))
' "$title" "$body")
    if ! curl -fsS --max-time 10 -X POST "$ALERT_HOOK" \
      -H 'Content-Type: application/json' \
      -d "$payload" > /dev/null 2>&1; then
      echo "  ❌ 告警发送失败" | tee -a "$LOG_FILE" >&2
    fi
  fi
}
