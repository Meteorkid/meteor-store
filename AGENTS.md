...
本地备份完成并**校验通过之后**，`backup-db.sh` 会调
[scripts/backup-to-r2.mjs](file:///Users/meteor/github/meteor-store/scripts/backup-to-r2.mjs)
传一份到 R2 的 `db-backups/` 前缀，保留天数与本地一致。

- **为什么要有**：迁到自建 PG 之后，应用与数据库已经是同一个单点。本地备份防得了
  「删错了 / 写坏了」，防不了「这台机器整体没了」
- **绝不能进 `R2_BUCKET`**：那个桶绑了公开域名（`R2_PUBLIC_BASE`），对象公开可读，
  而备份里有全部用户邮箱、密码哈希、订单与授权码。脚本只认私有桶——优先
  `R2_BACKUP_BUCKET`，否则回落到 `R2_RELEASE_BUCKET`；**两者都缺时拒绝执行**，
  桶名等于 `R2_BUCKET` 时同样拒绝。想要独立备份桶只需加一个环境变量，代码不动
- 上传后立刻 `HeadObject` 回读大小核对：PUT 返回 200 不代表对象落盘时是完整的
- R2 侧的过期清理放在**上传成功之后**。顺序反了的话，一次上传失败会把远端旧备份
  也清掉，本地和远端同时变空
- **上传失败不让整个备份失败**：本地那份已经写好并校验过，异地副本是加固不是前提。
  失败会往 stderr 留告警，cron 下进 syslog
- 容量：2026-09-02 实测 R2 已用 113 MB（安装包，占免费额度 1.13%），
  一份备份 276KB，每天一份存十年也用不到 10 GB 免费额度的 11%
- **做过一次完整的灾难恢复演练**：从 R2 下载 → `pg_restore` 到临时库 → 逐表核对，
  users 4 / orders 25 / posts 19 全部正确。改动备份链路后应重做一次
