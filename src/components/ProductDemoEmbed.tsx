    ...
    <section className="mb-20">
      <div className="mb-7 flex items-end justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-bold text-white md:text-3xl">{t('onlinePlayTitle')}</h2>
        </div>
        <a
          href={product.demo}
          target="_blank"
      ...
      <div className="relative aspect-[16/10] overflow-hidden rounded-[1.5rem] border border-white/10 bg-zinc-950">
        {loaded ? (
          <iframe
            src={product.demo}
            title={t('demoTitle', { name: product.name })}
          ...
          <button
            ...
            aria-label={t('loadDemoAria', { name: product.name })}
          >
            {product.media && (
              <Image
            ...
            <span className="relative mt-4 text-sm text-white/70">{t('loadDemo')}</span>
          </button>
        )}
      </div>

      <p className="mt-3 text-xs text-white/35">{t('demoLoadHint')}</p>
    </section>
  );
}
