...
export default async function PathfinderItemPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const related = sortByRecency(await listCatalogItems({ direction: item.direction }))
    ...
    <main className="container mx-auto px-4 py-10 sm:py-14 lg:py-16">
      <article className="mx-auto max-w-7xl">
        ...
        <header className="border-b border-white/10 pb-10">
          ...
          <h1 className="mt-5 max-w-5xl t-title-1 text-white">{localizedText(item.title, locale)}</h1>
          <p className="mt-4 max-w-4xl t-body text-white/60">{localizedText(item.summary, locale)}</p>
          ...
          <aside className="space-y-5">
            <section className="glass rounded-2xl p-5">
              ...
              <dl className="mt-4 divide-y divide-white/[0.07]">
                <CompactFact label={t('time')} value={item.estimatedMinutes === null ? t('unknown') : t('minutes', { minutes: item.estimatedMinutes })} />
                <CompactFact label={t('costLabel')} value={item.cost.amount === 0 ? t('free') : formatCatalogCost(item, typedLocale) ?? t('unknown')} />
                <CompactFact label={t('device')} value={t(`devices.${item.device}`)} />
                <CompactFact label={t('network')} value={t(`networks.${item.network}`)} />
                <CompactFact label={t('remoteLabel')} value={t(`remote.${item.remoteStatus}`)} />
                <CompactFact label={t('region')} value={item.region ? localizedText(item.region, locale) : t('unknown')} />
                <CompactFact label={t('deadlineLabel')} value={item.deadlineText ? localizedText(item.deadlineText, locale) : formatCatalogDeadlineDate(item, typedLocale) ?? t('unknown')} />
              </dl>
