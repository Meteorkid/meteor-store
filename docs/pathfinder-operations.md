...
- 可学习的动态条目连续 30 天未再次核验会转为 `stale`；AI 动态保留 180 天公开窗口，之后归档，避免公开目录无界增长。**资讯摘要（`digest` 来源）只保留 14 天**：日更来源按天累积，180 天窗口会攒出约 180 条日报、双语 360 个 sitemap URL——按整站 826 个 URL 算四成是过期新闻，而一份两个月前的 AI 日报没有回看价值。短窗口同时压掉部署后的冷启动面（日报详情页要现抓现渲染上游全文）。两档窗口由 `digest-content.test.ts` 钉住，防止有人改回一档。
